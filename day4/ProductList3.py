...
work_path = "C:/WORK/day4/"
#DB파일이 없으면 만들고 있다면 접속한다. 
if os.path.exists(work_path+"ProductList.db"):
    con = sqlite3.connect(work_path+"ProductList.db")
    cur = con.cursor()
else: 
    con = sqlite3.connect(work_path+"ProductList.db")
    cur = con.cursor()
...
#디자인 파일을 로딩
form_class = uic.loadUiType(work_path+"ProductList3.ui")[0]
